...
halted = False
crypto_registers = {"V0":0, "V1":0, "C0":0, "C1":0}
IF_ID = {}
ID_EX = {}
EX_MEM = {}
MEM_WB = {}
...
def load_program(program):
    global instr_memory, pc, halted, cycle_count, instruction_count, start_time, end_time
...
def step():
    global cycle_count, instruction_count
    print(f"\n========== Cycle {cycle_count} ==========")
    write_back()
    memory_access()
    execute()
    ...
    if op is not None and op != ISA['NOP']:
        instruction_count += 1
    cycle_count += 1

def run():
    global start_time, end_time, instruction_count, cycle_count, pipeline_registers, elapsed_time
